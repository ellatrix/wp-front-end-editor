(function ($, views, data, _, wp) {
  ...
  av = _.extend({}, media, {
    ...
    bindNode: function (editor, node) {
      ...
      if ($playlist.length) {
        return new window.WPPlaylistView({
          el: $playlist.get(0),
  ...
  embed = _.extend({}, av, {
    ...
    edit: function (text, update) {
      ...
      var frame = media.edit(text, this.url)
      var self = this
      var events = 'change:url change:width change:height'
